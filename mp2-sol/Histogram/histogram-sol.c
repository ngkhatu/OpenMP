...
long* histogram(char* fn_input) {
  ...
  histo = (long*) malloc(256*sizeof(long));
  lockArr = (omp_lock_t*) malloc(256*sizeof(omp_lock_t));
  for (i=0; i<256; i++)
    omp_init_lock(&lockArr[i]);
...
#pragma omp parallel for private(i,j)
	...
	omp_set_lock(&lockArr[image->content[i][j]]);
	histo[image->content[i][j]]++;
	omp_unset_lock(&lockArr[image->content[i][j]]);
      }
    }
  }
