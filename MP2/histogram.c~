...
omp_set_num_threads(1);
	...
	arrlock = malloc(256*sizeof(omp_lock_t));
	for(i=0;i<256;i++)omp_init_lock(&arrlock[i]);
	...
	for (m=0; m<10; m++) {
		...
		for (i=0; i<image->row; i++) {
			for (j=0; j<image->col; j++) {
				...
				//omp_set_lock(&arrlock[image->content[i][j]]);				
				#pragma omp atomic
				histo[image->content[i][j]]++;
				//omp_unset_lock(&arrlock[image->content[i][j]]);
			}
		}
	}

	free(arrlock);
	for(i=0;i<256;i++) omp_destroy_lock(&arrlock[i]);
